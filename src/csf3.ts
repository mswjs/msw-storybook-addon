...
function printDeprecationWarning() {
  if (!hasPrintedDeprecationWarning) {
    console.warn(
      '[msw-storybook-addon] `mswLoader` is deprecated when using CSF Next — use `addonMsw()` instead. Run `npx msw-storybook-addon migrate` to migrate.\n\nLearn more: https://github.com/mswjs/msw-storybook-addon/blob/main/MIGRATION.md#from-2xx-to-3xx'
    )
 ...
 * instead. Run `npx msw-storybook-addon migrate` to migrate automatically. See the
